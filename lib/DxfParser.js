...
DxfParser.prototype._parse = function(dxfString) {
	 ...
	 * 0:SPLINE. Scanner.next() will return the first attribute of the
	 * entity.
	 * @return {Object} the entity parsed
	 */
	var parseSPLINE = function() {
		var entity = { type: curr.value, knots: [] },
			numberOfKnots = 0;
		curr = scanner.next();
		while(curr !== 'EOF') {
			if(curr.code === 0) break;
			...
			switch(curr.code) {
				case 100:	// Subclass marker
					entity.subclass = curr.value;
					curr = scanner.next();
					break;
				case 210:	// Normal vector X
					entity.normalVectorX = curr.value;
					curr = scanner.next();
					break;
				case 220:	// Normal vector Y
					entity.normalVectorZ = curr.value;
					curr = scanner.next();
					break;
				case 230:	// Normal vector Z
					entity.normalVectorZ = curr.value;
					curr = scanner.next();
					break;
				case 70: 	// 1 = Closed, 2 = Periodic, 4 = Rational, 8 = Planar, 16 = Linear
					entity.closedSpline = (curr.value & 1 > 0);
					entity.periodicSpline = (curr.value & 2 > 0);
					entity.rationalSpline = (curr.value & 4 > 0);
					entity.planarSpline = (curr.value & 8 > 0);
					entity.linearSpline = (curr.value & 8 > 0);
					curr = scanner.next();
					break;
				case 71:	// Degree of the spline curve
					entity.degreeOfCurve = curr.value;
					curr = scanner.next();
					break;
				...
				case 42:	// Knot tolerance
					entity.knotTolerance = curr.value;
					curr = scanner.next();
					break;
				case 43:	// Control-point tolerance
					entity.controlPointTolerance = curr.value;
					curr = scanner.next();
					break;
				case 44:	// Fit tolerance
					entity.fitTolerance = curr.value;
					curr = scanner.next();
					break;
				case 40:	// Knot values
					parseSplineKnots(numberOfKnots)
					curr = scanner.next();
					break;
				default:
					checkCommonEntityProperties(entity);
	...
	var parseSplineKnots = function(n) {
		if(!n || n <= 0) throw Error('n must be greater than 0 knots');
		var vertices = [], i;
		var vertexIsStarted = false;
		var vertexIsFinished = false;
